 ...
 * <p/>
 * This class borrowed some of its methods from a modified fork of the <a href=
 * "http://svn.apache.org/repos/asf/harmony/enhanced/java/branches/java6/classlib/modules/luni/
 * src/main/java/org/apache/harmony/luni/util/Inet6Util.java">Inet6Util
 * class</a> which was part of Apache Harmony.
 */
public final class NetUtil {

	/**
	 * This defines how many words (represented as ints) are needed to represent
	 * an IPv6 address
	 */
	...
	/**
	 * Returns the {@link Inet6Address} representation of a {@link CharSequence}
	 * IP address.
	 * <p>
	 * This method will treat all IPv4 type addresses as "IPv4 mapped" (see
	 * {@link #getByName(CharSequence, boolean)})
	 * 
	 * @param ip
	 *            {@link CharSequence} IP address to be converted to a
	 *            {@link Inet6Address}
	 * @return {@link Inet6Address} representation of the {@code ip} or
	 *         {@code null} if not a valid IP address.
	 */
	 ...
	 * IP address.
	 * <p>
	 * The {@code ipv4Mapped} parameter specifies how IPv4 addresses should be
	 * treated. "IPv4 mapped" format as defined in
	 * <a href="http://tools.ietf.org/html/rfc4291#section-2.5.5">rfc 4291
	 * section 2</a> is supported.
	 * 
	 * @param ip
	 *            {@link CharSequence} IP address to be converted to a
	 *            {@link Inet6Address}
	 * @param ipv4Mapped
	 *            <ul>
	 *            <li>{@code true} To allow IPv4 mapped inputs to be translated
	 *            into {@link Inet6Address}</li>
	 *            <li>{@code false} Don't turn IPv4 addressed to mapped
	 *            addresses</li>
	 *            </ul>
	 * @return {@link Inet6Address} representation of the {@code ip} or
	 *         {@code null} if not a valid IP address.
	...
	public static Inet6Address getByName(CharSequence ip, boolean ipv4Mapped) {
		...
		for (; i < ipLength; ++i) {
			...
			case ':':
				++ipv6Seperators;
				if (i - begin > IPV6_MAX_CHAR_BETWEEN_SEPARATOR || ipv4Seperators > 0 || ipv6Seperators > IPV6_MAX_SEPARATORS || currentIndex + 1 >= bytes.length) {
					return null;
				}
				...
				// The value integer holds at most 4 bytes from right (most
				// significant) to left (least significant).
				// The following bit shifting is used to extract and re-order
				// the individual bytes to achieve a
			...
			case '.':
				++ipv4Seperators;
				if (i - begin > IPV4_MAX_CHAR_BETWEEN_SEPARATOR || ipv4Seperators > IPV4_SEPARATORS || (ipv6Seperators > 0 && (currentIndex + compressLength < 12)) || i + 1 >= ipLength || currentIndex >= bytes.length || begin < 0 || (begin == 0 && (i == 3 && (!isValidNumericChar(ip.charAt(2)) || !isValidNumericChar(ip.charAt(1)) || !isValidNumericChar(ip.charAt(0))) || i == 2 && (!isValidNumericChar(ip.charAt(1)) || !isValidNumericChar(ip.charAt(0))) || i == 1 && !isValidNumericChar(ip.charAt(0))))) {
					return null;
				}
				value <<= (IPV4_MAX_CHAR_BETWEEN_SEPARATOR - (i - begin)) << 2;

				// The value integer holds at most 3 bytes from right (most
				// significant) to left (least significant).
				// The following bit shifting is to restructure the bytes to be
				// left (most significant) to
				// right (least significant) while also accounting for each IPv4
				// digit is base 10.
			...
			default:
				...
				// The value is treated as a sort of array of numbers because we
				// are dealing with
				// at most 4 consecutive bytes we can use bit shifting to
				// accomplish this.
				// The most significant byte will be encountered first, and
				// reside in the right most
		...
		// Finish up last set of data that was accumulated in the loop (or
		// before the loop)
		if (ipv4Seperators > 0) {
			if (begin > 0 && i - begin > IPV4_MAX_CHAR_BETWEEN_SEPARATOR || ipv4Seperators != IPV4_SEPARATORS || currentIndex >= bytes.length) {
				return null;
			}
			if (ipv6Seperators == 0) {
				compressLength = 12;
			} else if (ipv6Seperators >= IPV6_MIN_SEPARATORS && ip.charAt(ipLength - 1) != ':' && (!isCompressed && (ipv6Seperators == 6 && ip.charAt(0) != ':') || isCompressed && (ipv6Seperators + 1 < IPV6_MAX_SEPARATORS && (ip.charAt(0) != ':' || compressBegin <= 2)))) {
				compressLength -= 2;
			} else {
				return null;
			}
			...
			// The value integer holds at most 3 bytes from right (most
			// significant) to left (least significant).
			// The following bit shifting is to restructure the bytes to be left
			// (most significant) to
			// right (least significant) while also accounting for each IPv4
			// digit is base 10.
			begin = (value & 0xf) * 100 + ((value >> 4) & 0xf) * 10 + ((value >> 8) & 0xf);
			...
			tmp = ipLength - 1;
			if (begin > 0 && i - begin > IPV6_MAX_CHAR_BETWEEN_SEPARATOR || ipv6Seperators < IPV6_MIN_SEPARATORS || !isCompressed && (ipv6Seperators + 1 != IPV6_MAX_SEPARATORS || ip.charAt(0) == ':' || ip.charAt(tmp) == ':') || isCompressed && (ipv6Seperators > IPV6_MAX_SEPARATORS || (ipv6Seperators == IPV6_MAX_SEPARATORS && (compressBegin <= 2 && ip.charAt(0) != ':' || compressBegin >= 14 && ip.charAt(tmp) != ':'))) || currentIndex + 1 >= bytes.length) {
				return null;
			}
			...
			// The value integer holds at most 4 bytes from right (most
			// significant) to left (least significant).
			// The following bit shifting is used to extract and re-order the
			// individual bytes to achieve a
	 ...
	 * <ul>
	 * <li>Inet4Address results are identical to
	 * {@link InetAddress#getHostAddress()}</li>
	 * <li>Inet6Address results adhere to
	 * <a href="http://tools.ietf.org/html/rfc5952#section-4">rfc 5952 section
	 * 4</a></li>
	 * </ul>
	 * <p>
	 ...
	 * 
	 * @param ip
	 *            {@link InetAddress} to be converted to an address string
	 ...
	 * <li>Inet6Address results adhere to
	 * <a href="http://tools.ietf.org/html/rfc5952#section-4">rfc 5952 section
	 * 4</a> if {@code ipv4Mapped} is false. If {@code ipv4Mapped} is true then
	 * "IPv4 mapped" format from
	 * <a href="http://tools.ietf.org/html/rfc4291#section-2.5.5">rfc 4291
	 * section 2</a> will be supported. The compressed result will always obey
	 * the compression rules defined in
	 * <a href="http://tools.ietf.org/html/rfc5952#section-4">rfc 5952 section
	 * 4</a></li>
	 * </ul>
	 * <p>
	 ...
	 * @param ipv4Mapped
	 *            <ul>
	 *            <li>{@code true} to stray from strict rfc 5952 and support the
	 *            "IPv4 mapped" format defined in
	 *            <a href="http://tools.ietf.org/html/rfc4291#section-2.5.5">rfc
	 *            4291 section 2</a> while still following the updated
	 *            guidelines in
	 *            <a href="http://tools.ietf.org/html/rfc5952#section-4">rfc
	 *            5952 section 4</a></li>
	...
	public static String toAddressString(InetAddress ip, boolean ipv4Mapped) {
		...
		if (shortestEnd < 0) { // Optimization when there is no compressing
								// needed
		...
		} else { // General case that can handle compressing (and not
					// compressing)
			// Loop unroll the first index (so we don't constantly check i==0
			// cases in loop)
			...
			for (i = 1; i < words.length; ++i) {
				if (!inRangeEndExclusive(i, shortestStart, shortestEnd)) {
					if (!inRangeEndExclusive(i - 1, shortestStart, shortestEnd)) {
						// If the last index was not part of the shortened
						// sequence
						if (!isIpv4Mapped || i == 6) {
							b.append(':');
						} else {
					...
					// If we are in the shortened sequence and the last index
					// was not
					b.append("::");
				}
			}
	...
	/**
	 * Does a range check on {@code value} if is within {@code start}
	 * (inclusive) and {@code end} (exclusive).
	 * 
	 * @param value
	 *            The value to checked if is within {@code start} (inclusive)
	 ...
	 * @return
	 *         <ul>
	 *         <li>{@code true} if {@code value} if is within {@code start}
	 *         (inclusive) and {@code end} (exclusive)</li>
