...
import com.freeswitch.netty.util.internal.DetectionUtil;
...
public class HashedWheelTimer implements Timer {
	...
	private volatile int workerState = WORKER_STATE_INIT; // 0 - init, 1 -
															// started, 2 - shut
															// down
	 ...
	 *            execution.
	 * @param determiner
	 *            thread name determiner to control thread name.
	 * @param tickDuration
	 *            the duration between tick
	 * @param unit
	 *            the time unit of the {@code tickDuration}
	...
	private final class Worker implements Runnable {
		...
		private void transferTimeoutsToBuckets() {
			...
			for (int i = 0; i < 100000; i++) {
				...
				if (timeout.state() == HashedWheelTimeout.ST_CANCELLED || !timeout.compareAndSetState(HashedWheelTimeout.ST_INIT, HashedWheelTimeout.ST_IN_BUCKET)) {
					// Was cancelled in the meantime. So just remove it and
					// continue with next HashedWheelTimeout
					// in the queue
					timeout.remove();
					continue;
				}
	...
	private static final class HashedWheelTimeout implements Timeout {
		...
		public void cancel() {
			...
			if (state != ST_IN_BUCKET && compareAndSetState(ST_INIT, ST_CANCELLED)) {
				// Was cancelled before the HashedWheelTimeout was added to its
				// HashedWheelBucket.
				// In this case we can just return here as it will be discarded
				// by the WorkerThread when handling
			...
			// Add the HashedWheelTimeout back to the timeouts queue so it will
			// be picked up on the next tick
			// and remove this HashedTimeTask from the HashedWheelBucket. After
			// this is done it is ready to get
	...
	private static final class HashedWheelBucket {
		...
		public void expireTimeouts(long deadline) {
			...
			while (timeout != null) {
				...
				// store reference to next as we may null out timeout.next in
				// the remove block.
