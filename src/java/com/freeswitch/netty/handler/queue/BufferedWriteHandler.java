 ...
 * </pre>
 *
 * <h3>Auto-flush</h3> The write request queue is automatically flushed when the
 * associated {@link Channel} is disconnected or closed. However, it does not
 * flush the queue otherwise. It means you have to call {@link #flush()} before
 * the size of the queue increases too much. You can implement your own
 ...
 * <p>
 * The following server implements the consolidation strategy that reduces the
 * number of write requests based on the writability of a channel:
 * 
 * <pre>
 ...
 * queue in the constructor of this handler. It will be required to design the
 * proper strategy to determine how often {@link #flush()} should be called. For
 * server, you could call {@link #flush()} periodically, using
...
public class BufferedWriteHandler extends SimpleChannelHandler implements LifeCycleAwareChannelHandler {
	 ...
	 *            {@code true} if and only if the buffered write requests are
	 *            merged into a single write request on {@link #flush()}
	 */
	...
	/**
	 * Creates a new instance with the specified thread-safe unbounded
	 * {@link Queue}. Please note that specifying a bounded {@link Queue} or a
	 * thread-unsafe {@link Queue} will result in an unspecified behavior.
	 *
	 * @param consolidateOnFlush
	 *            {@code true} if and only if the buffered write requests are
	 *            merged into a single write request on {@link #flush()}
	 ...
	 * @param consolidateOnFlush
	 *            {@code true} if and only if the buffered write requests are
	 *            merged into a single write request
	 */
	...
	/**
	 * Fail all buffered writes that are left. See <a
	 * href="https://github.com/netty/netty/issues/308>#308</a> for more
	 * details.
	 */
	@Override
