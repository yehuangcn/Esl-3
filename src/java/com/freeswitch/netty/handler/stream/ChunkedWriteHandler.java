...
import static com.freeswitch.netty.channel.Channels.future;
 ...
 * {@link OutOfMemoryError}. Large data streaming such as file transfer requires
 * complicated state management in a {@link ChannelHandler} implementation.
 * {@link ChunkedWriteHandler} manages such complicated states so that you can
 * send a large data stream without difficulties.
 * <p>
 * To use {@link ChunkedWriteHandler} in your application, you have to insert a
 * new {@link ChunkedWriteHandler} instance:
 * 
 * <pre>
...
public class ChunkedWriteHandler implements ChannelUpstreamHandler, ChannelDownstreamHandler, LifeCycleAwareChannelHandler {
	...
	public void handleDownstream(ChannelHandlerContext ctx, ChannelEvent e) throws Exception {
		...
		// call flush if the channel is writable or not connected. flush(..)
	...
	private void flush(ChannelHandlerContext ctx, boolean fireNow) throws Exception {
		...
		flushNeeded = true;
		// use CAS to see if the have flush already running, if so we don't need
		// to take futher actions
			...
			try {
				...
				while (channel.isWritable()) {
					...
					if (currentEvent.getFuture().isDone()) {
						// Skip the current request because the previous partial
						// write
						...
						if (m instanceof ChunkedInput) {
							...
							try {
								...
								if (chunk == null) {
									...
									// No need to suspend when reached at the
									// end.
									suspend = !endOfInput;
								} else {
									suspend = false;
							...
							if (suspend) {
								// ChunkedInput.nextChunk() returned null and it
								// has
								// not reached at the end of input. Let's wait
								// until
								// more chunks arrive. Nothing to write or
								// notify.
								break;
							} else {
								...
								if (endOfInput) {
									...
									// Register a listener which will close the
									// input once the write
									// is complete. This is needed because the
									// Chunk may have some
									// resource bound that can not be closed
									// before its not written
									//
									// See
									// https://github.com/netty/netty/issues/303
	...
	// This method should not need any synchronization as the
	// ChunkedWriteHandler will not receive any new events
	public void afterRemove(ChannelHandlerContext ctx) throws Exception {
		// Fail all MessageEvent's that are left. This is needed because
		// otherwise we would never notify the
