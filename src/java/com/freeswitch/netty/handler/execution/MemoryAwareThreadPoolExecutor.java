...
import com.freeswitch.netty.util.internal.ConcurrentIdentityHashMap;
import com.freeswitch.netty.util.internal.SharedResourceMisuseDetector;

/**
 * A {@link ThreadPoolExecutor} which blocks the task submission when there's
 * too many tasks in the queue. Both per-{@link Channel} and
 * per-{@link Executor} limitation can be applied.
 * <p>
 * When a task (i.e. {@link Runnable}) is submitted,
 * {@link MemoryAwareThreadPoolExecutor} calls
 * {@link ObjectSizeEstimator#estimateSize(Object)} to get the estimated size of
 * the task in bytes to calculate the amount of memory occupied by the
 * unprocessed tasks.
 * <p>
 ...
 * <li>you are submitting a task whose type is not {@link ChannelEventRunnable},
 * or</li>
 * <li>the message type of the {@link MessageEvent} in the
 * {@link ChannelEventRunnable} is not {@link ChannelBuffer}.</li>
 * </ul>
 * Here is an server that demonstrates how to implement an
 * {@link ObjectSizeEstimator} which understands a user-defined object:
 * 
 * <pre>
 ...
 * {@link ChannelEvent}s for the same {@link Channel}. For server, you can even
 * receive a {@code "channelClosed"} event before a {@code "messageReceived"}
 * event, as depicted by the following diagram.
 *
 * For server, the events can be processed as depicted below:
 *
 * <pre>
 *           --------------------------------&gt; Timeline --------------------------------&gt;
 *
 ...
 * </pre>
 *
 * To maintain the event order, you must use
 * {@link OrderedMemoryAwareThreadPoolExecutor}.
 *
 * @apiviz.has org.jboss.netty.util.ObjectSizeEstimator oneway - -
 * @apiviz.has org.jboss.netty.handler.execution.ChannelEventRunnable oneway - -
 *             executes
 */
public class MemoryAwareThreadPoolExecutor extends ThreadPoolExecutor {
	...
	/**
	 * This will call {@link #shutdownNow(boolean)} with the value of
	 * {@link #getNotifyChannelFuturesOnShutdown()}.
	 */
	@Override
	...
	/**
	 * See {@link ThreadPoolExecutor#shutdownNow()} for how it handles the
	 * shutdown. If {@code true} is given to this method it also notifies all
	 * {@link ChannelFuture}'s of the not executed
	 * {@link ChannelEventRunnable}'s.
	 *
	 * <p>
	 * Be aware that if you call this with {@code false} you will need to handle
	 * the notification of the {@link ChannelFuture}'s by your self. So only use
	 * this if you really have a use-case for it.
	 * </p>
	 *
	 */
	public List<Runnable> shutdownNow(boolean notify) {
		...
		// loop over all tasks and cancel the ChannelFuture of the
		// ChannelEventRunable's
		for (Runnable task : tasks) {
			if (task instanceof ChannelEventRunnable) {
				...
				// store the Channel of the event for later notification of the
	...
	/**
	 * Sets the maximum total size of the queued events per channel. Specify
	 * {@code 0} to disable.
	 */
	...
	/**
	 * If set to {@code false} no queued {@link ChannelEventRunnable}'s
	 * {@link ChannelFuture} will get notified once {@link #shutdownNow()} is
	 * called. If set to {@code true} every queued {@link ChannelEventRunnable}
	 * will get marked as failed via
	 * {@link ChannelFuture#setFailure(Throwable)}.
	 *
	 * <p>
	 * Please only set this to {@code false} if you want to handle the
	 * notification by yourself and know what you are doing. Default is
	 * {@code true}.
	 * </p>
	 */
	...
	/**
	 * Returns if the {@link ChannelFuture}'s of the
	 * {@link ChannelEventRunnable}'s should be notified about the shutdown of
	 * this {@link MemoryAwareThreadPoolExecutor}.
	 */
		...
		if (task instanceof ChannelEventRunnable) {
			...
			if (maxChannelMemorySize != 0 && channelCounter < maxChannelMemorySize && channel.isOpen()) {
				if (!channel.isReadable()) {
					...
					ChannelHandlerContext ctx = eventTask.getContext();
					if (ctx.getHandler() instanceof ExecutionHandler) {
						// check if the attachment was set as this means that we
						// suspend the channel
						// from reads. This only works when this pool is used
	...
	/**
	 * Returns {@code true} if and only if the specified {@code task} should be
	 * counted to limit the global and per-channel memory consumption. To
	 * override this method, you must call {@code super.shouldCount()} to make
	 * sure important tasks are not counted.
	 */
