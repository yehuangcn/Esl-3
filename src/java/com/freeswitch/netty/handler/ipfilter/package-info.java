 ...
 * proposes to allow only one connection by client's IP Address. I.E. this
 ...
 * <tt>IpV4SubnetFilterRule</tt> (IPV4 support only),
 ...
 * on this channelFuture</b> (at least with respect of asynchronous
 * operations).<br>
 * <br>
 ...
 * pipeline if any, and False (default) if no events has to be passed to the
 * next handlers when a channel is blocked. This is intend to prevent any
 * unnecessary action since the connection is refused.<br>
 * However, you could change its behavior for instance because you don't want
 * that any event will be blocked by this filter by returning always true or
 * according to some events.<br>
 * <b>Note that OPENED and BOUND events are still passed to the next entry in
 * the pipeline since those events come out before the CONNECTED event, so there
 * is no possibility to filter those two events before the CONNECTED event shows
 * up. Therefore, you might want to let CLOSED and UNBOUND be passed to the next
 * entry in the pipeline.</b><br>
 * <br>
 * Per default implementation this method invokes an
 ...
 * the channel), then any new events on this channel will be blocked.<br>
 * However, you could change its behavior for instance because you don't want
 * that all events will be blocked by this filter by testing the result of
