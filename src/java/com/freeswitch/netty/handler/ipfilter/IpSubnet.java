...
/**
 * This class allows to check if an IP V4 or V6 Address is contained in a
 * subnet.<BR>
 * <p/>
 * Supported IP V4 Formats for the Subnets are: 1.1.1.1/255.255.255.255 or
 * 1.1.1.1/32 (CIDR-Notation) and (InetAddress,Mask) where Mask is a integer for
 * CIDR-notation or a String for Standard Mask notation.<BR>
 * <BR>
 * <BR>
 * Example1:<BR>
 ...
 * where inetAddress is 192.168.1.0 and inetAddress2 is 192.168.1.123<BR>
 * <BR>
 * Supported IP V6 Formats for the Subnets are: a:b:c:d:e:f:g:h/NN
 * (CIDR-Notation) or any IPV6 notations (like a:b:c:d::/NN,
 * a:b:c:d:e:f:w.x.y.z/NN) and (InetAddress,Mask) where Mask is a integer for
 * CIDR-notation and (InetAddress,subnet).<BR>
 * <BR>
 * <BR>
 * Example1:<BR>
...
public class IpSubnet implements IpSet, Comparable<IpSubnet> {
	...
	/**
	 * Compares the given InetAddress against the Subnet and returns true if the
	 * ip is in the subnet-ip-range and false if not.
	 *
	 * @return returns true if the given IP address is inside the currently set
	 *         network.
