...
import com.freeswitch.netty.handler.codec.frame.TooLongFrameException;
import com.freeswitch.netty.handler.codec.http.HttpMessageDecoder.State;
import com.freeswitch.netty.handler.codec.replay.ReplayingDecoder;
 ...
 * be split into multiple {@link HttpChunk}s whose length is
 ...
 * single HTTP message to avoid excessive memory consumption. For server, the
 ...
 * <li>The first {@link HttpChunk} whose content is
 * {@code 'abcdefghijklmnopqrstuvwxyz'},</li>
...
public abstract class HttpMessageDecoder extends ReplayingDecoder<State> {
	...
	/**
	 * The internal state of {@link HttpMessageDecoder}. <em>Internal use
	 * only</em>.
	 * 
	 * @apiviz.exclude
	 */
	protected enum State {
		SKIP_CONTROL_CHARS, READ_INITIAL, READ_HEADER, READ_VARIABLE_LENGTH_CONTENT, READ_VARIABLE_LENGTH_CONTENT_AS_CHUNKS, READ_FIXED_LENGTH_CONTENT, READ_FIXED_LENGTH_CONTENT_AS_CHUNKS, READ_CHUNK_SIZE, READ_CHUNKED_CONTENT, READ_CHUNKED_CONTENT_AS_CHUNKS, READ_CHUNK_DELIMITER, READ_CHUNK_FOOTER, UPGRADED
	}
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer, State state) throws Exception {
		...
		case READ_FIXED_LENGTH_CONTENT_AS_CHUNKS: {
			...
			// count
			// to create the HttpChunk. This is needed as otherwise we may end
			// up with
			// create a HttpChunk instance that contains an empty buffer and so
			// is
		 ...
		 * content. basically, read chunk size, read chunk, read and ignore the
		 * CRLF and repeat until 0
		 */
		...
		case UPGRADED: {
			...
			if (readableBytes > 0) {
				...
				// DecoderException,
				// other handler will replace this codec with the upgraded
				// protocol codec to
	...
	protected boolean isContentAlwaysEmpty(HttpMessage msg) {
		if (msg instanceof HttpResponse) {
			...
			//
			// See:
			// - http://www.w3.org/Protocols/rfc2616/rfc2616-sec4.html Section
			// 4.4
