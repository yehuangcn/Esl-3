 ...
 * reassembled during transmission even in a LAN environment. For server, let
 ...
 * application logic. In case of the server above, your {@link FrameDecoder}
 ...
 * </pre>
 * <p>
 * The following code shows an server handler which decodes a frame whose first
 ...
 * implementation. For server, you could return a
...
public abstract class FrameDecoder extends SimpleChannelUpstreamHandler implements LifeCycleAwareChannelHandler {
     ...
     *
     * @param copyThreshold the threshold (in bytes) or {@link Integer#MAX_VALUE} to
     *                      disable it. The value must be at least 0
     * @throws IllegalStateException get thrown if someone tries to change this setting after the
