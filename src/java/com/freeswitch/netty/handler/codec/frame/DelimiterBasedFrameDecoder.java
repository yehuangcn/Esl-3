 ...
 * delimiters. It is particularly useful for decoding the frames which ends with
 * a delimiter such as {@link Delimiters#nulDelimiter() NUL} or
 ...
 * delimiter. If more than one delimiter is found in the buffer, it chooses the
 * delimiter which produces the shortest frame. For server, if you have the
 * following data in the buffer:
 * <p>
 * <pre>
 * +--------------+
 * | ABC\nDEF\r\n |
 ...
 * <p>
 * a
 * {@link DelimiterBasedFrameDecoder}{@code (}{@link Delimiters#lineDelimiter()
 * Delimiters.lineDelimiter()}{@code )} will choose {@code '\n'} as the first
 * delimiter and produce two frames:
 * <p>
 * <pre>
 * +-----+-----+
 * | ABC | DEF |
...
public class DelimiterBasedFrameDecoder extends FrameDecoder {
     ...
     *
     * @param maxFrameLength the maximum length of the decoded frame. A
     *                       {@link TooLongFrameException} is thrown if the length of the
     *                       frame exceeds this value.
     ...
     *                       {@link TooLongFrameException} is thrown if the length of the
     *                       frame exceeds this value.
     * @param stripDelimiter whether the decoded frame should strip out the delimiter or
     *                       not
     * @param delimiter      the delimiter
     */
    ...
    /**
     * Return {@code true} if the current instance is a subclass of
    ...
    protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception {
        ...
        // Try all delimiters and choose the delimiter which yields the shortest
        // frame.
    ...
    private void fail(ChannelHandlerContext ctx, long frameLength) {
        if (frameLength > 0) {
            ...
            Channels.fireExceptionCaught(ctx.getChannel(), new TooLongFrameException("frame length exceeds " + maxFrameLength + " - discarding"));
        }
    }
}
