 ...
 * {@link ChannelTrafficShapingHandler}), as traffic shaping. It allows too to
 * implement an almost real time monitoring of the bandwidth using the monitors
 * from {@link TrafficCounter} that will call back every checkInterval the
 * method doAccounting of this handler.
 * </p>
 * <p>
 * <p>
 * An {@link ObjectSizeEstimator} can be passed at construction to specify what
 * is the size of the object to be read or write accordingly to the type of
 * object. If not specified, it will used the {@link DefaultObjectSizeEstimator}
 * implementation.
 * </p>
 * <p>
 * <p>
 * If you want for any particular reasons to stop the monitoring (accounting) or
 * to change the read/write limit or the check interval, several methods allow
 * that for you:
 * </p>
 * <ul>
 ...
 * TrafficCounter and so to stop or start the monitoring, to change the
 * checkInterval directly, or to have access to its values.</li>
 * </ul>
 */
public abstract class AbstractTrafficShapingHandler extends SimpleChannelHandler implements ExternalResourceReleasable {
    ...
    /**
     * Default max delay in case of traffic shaping (during which no
     * communication will occur). Shall be less than TIMEOUT. Here half of
     * "standard" 30s
     */
    ...
    /**
     * Boolean associated with the release of this TrafficShapingHandler. It
     * will be true only once when the releaseExternalRessources is called to
     * prevent waiting when shutdown.
     */
     ...
     */
    protected volatile long checkInterval = DEFAULT_CHECK_INTERVAL; // default 1
    /**
     * Max delay in wait
     */
    ...
    /**
     * Max size in the list before proposing to stop writing new objects from
     * next handlers
     */
    ...
    /**
     * Constructor using default {@link ObjectSizeEstimator} and default max
     * time as delay allowed value of {@value #DEFAULT_MAX_TIME} ms.
     *
     * @param timer         created once for instance like HashedWheelTimer(10,
    ...
    /**
     * Constructor using the specified ObjectSizeEstimator and default max time
     * as delay allowed value of {@value #DEFAULT_MAX_TIME} ms.
     *
     * @param objectSizeEstimator the {@link ObjectSizeEstimator} that will be used to compute
    ...
    /**
     * Constructor using default {@link ObjectSizeEstimator} and using default
     * Check Interval value of {@value #DEFAULT_CHECK_INTERVAL} ms and default
     * max time as delay allowed value of {@value #DEFAULT_MAX_TIME} ms.
     *
     * @param timer      created once for instance like HashedWheelTimer(10,
     *                   TimeUnit.MILLISECONDS, 1024).
     * @param writeLimit 0 or a limit in bytes/s
    ...
    protected AbstractTrafficShapingHandler(Timer timer, long writeLimit, long readLimit) {
        index = userDefinedWritabilityIndex();
        init(new SimpleObjectSizeEstimator(), timer, writeLimit, readLimit, DEFAULT_CHECK_INTERVAL, DEFAULT_MAX_TIME);
    }

    /**
     * Constructor using the specified ObjectSizeEstimator and using default
     * Check Interval value of {@value #DEFAULT_CHECK_INTERVAL} ms and default
     * max time as delay allowed value of {@value #DEFAULT_MAX_TIME} ms.
     *
     * @param objectSizeEstimator the {@link ObjectSizeEstimator} that will be used to compute
    ...
    protected AbstractTrafficShapingHandler(ObjectSizeEstimator objectSizeEstimator, Timer timer, long writeLimit, long readLimit) {
        index = userDefinedWritabilityIndex();
        init(objectSizeEstimator, timer, writeLimit, readLimit, DEFAULT_CHECK_INTERVAL, DEFAULT_MAX_TIME);
    }

    /**
     * Constructor using default {@link ObjectSizeEstimator} and using NO LIMIT
     * and default Check Interval value of {@value #DEFAULT_CHECK_INTERVAL} ms
     * and default max time as delay allowed value of {@value #DEFAULT_MAX_TIME}
     * ms.
     *
     * @param timer created once for instance like HashedWheelTimer(10,
     *              TimeUnit.MILLISECONDS, 1024).
    ...
    protected AbstractTrafficShapingHandler(Timer timer) {
        index = userDefinedWritabilityIndex();
        init(new SimpleObjectSizeEstimator(), timer, 0, 0, DEFAULT_CHECK_INTERVAL, DEFAULT_MAX_TIME);
    }
    ...
    protected AbstractTrafficShapingHandler(ObjectSizeEstimator objectSizeEstimator, Timer timer) {
        index = userDefinedWritabilityIndex();
        init(objectSizeEstimator, timer, 0, 0, DEFAULT_CHECK_INTERVAL, DEFAULT_MAX_TIME);
    }
     ...
     * @param checkInterval The delay between two computations of performances for
     *                      channels or 0 if no stats are to be computed.
     * @param maxTime       The max time to wait in case of excess of traffic (to prevent
    ...
    /**
     * @return the index to be used by the TrafficShapingHandler to manage the
     * user defined writability. For Channel TSH it is defined as
     * {@value #CHANNEL_DEFAULT_USER_DEFINED_WRITABILITY_INDEX}, for
     * Global TSH it is defined as
     * {@value #GLOBAL_DEFAULT_USER_DEFINED_WRITABILITY_INDEX}, for
     * GlobalChannel TSH it is defined as
     ...
     * <p>
     * Note the change will be taken as best effort, meaning that all already
     * scheduled traffics will not be changed, but only applied to new traffics.
     * </p>
     ...
     * accordingly to the traffic shaping configuration.
     *
     * @param maxTime Max delay in wait, shall be less than TIME OUT in related
     *                protocol. Must be positive.
     */
     ...
     * accordingly to the traffic shaping configuration.
     *
     * @param maxWriteDelay the maximum Write Delay in ms in the buffer allowed before
     *                      write suspended is set. Must be positive.
     */
     ...
     * accordingly to the traffic shaping configuration.
     *
     * @param maxWriteSize the maximum Write Size allowed in the buffer per channel
     *                     before write suspended is set, default being
     *                     {@value #DEFAULT_MAX_SIZE} bytes
     */
            ...
            if (size > 0 && trafficCounter != null) {
                ...
                if (wait >= MINIMAL_WAIT) { // At least 10ms seems a minimal
                    ...
                    if (channel != null && channel.isConnected()) {
                        // Only AutoRead AND HandlerActive True means Context
                        // Active
                        ...
                        if (timer == null) {
                            ...
                            // logger.warn("Read sleep since no timer for
                            // "+wait+" ms for "+this);
                            Thread.sleep(wait);
                            return;
                        }
                        if (channel.isReadable() && !rws.readSuspend) {
                            ...
                            // Create a Runnable to reactive the read if needed.
                            // If one was create before
                            ...
                            timeout = timer.newTimeout(rws.reopenReadTimerTask, wait, TimeUnit.MILLISECONDS);
                        }
                    }
                }
            }
    ...
    public void writeRequested(ChannelHandlerContext ctx, MessageEvent evt) throws Exception {
        ...
        try {
            if (size > 0 && trafficCounter != null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Write suspend: " + wait + ':' + channel.isWritable() + ':' + channel.getUserDefinedWritability(index));
                }
    ...
    /**
     * Check the writability according to delay and size for the channel. Set if
     * necessary the write suspended status.
     *
    ...
    class ReopenReadTimerTask implements TimerTask {
        ...
        public void run(Timeout timeoutArg) throws Exception {
            ...
            if (!channel.isReadable() && !rws.readSuspend) {
                // If isReadable is False and Active is True, user make a direct
                // setReadable(false)
