 ...
 * </p>
 * <p>
 * A TrafficCounter has for goal to count the traffic in order to enable to
 * limit the traffic or not, globally or per channel. It compute statistics on
 * read and written bytes at the specified interval and call back the
 * {@link AbstractTrafficShapingHandler} doAccounting method at every specified
 * interval. If this interval is set to 0, therefore no accounting will be done
 * and only statistics will be computed at each receive or write operations.
 */
public class TrafficCounter {
    ...
    /**
     * Constructor with the {@link AbstractTrafficShapingHandler} that hosts it,
     * the Timer to use, its name, the checkInterval between two computations in
     * millisecond.
     *
     * @param trafficShapingHandler the associated AbstractTrafficShapingHandler
     * @param timer                 Could be a HashedWheelTimer, might be null when used from
    ...
    public void start() {
        ...
        // if executor is null, it means it is piloted by a
        // GlobalChannelTrafficCounter, so no executor
    ...
    /**
     * @return the current checkInterval between two computations of traffic
     * counter in millisecond.
     */
    ...
    /**
     * @return the Read Throughput in bytes/s computes in the last check
     * interval.
     */
    ...
    /**
     * @return the Write Throughput in bytes/s computes in the last check
     * interval.
     */
    ...
    /**
     * @return the current number of bytes written since the last check
     * Interval.
     */
     ...
     * System.currentTimeMillis() when the cumulative counters were
     * reset to 0.
     */
