 ...
 * global and per channel traffic shaping, that is to say a global limitation of
 * the bandwidth, whatever the number of opened channels and a per channel
 * limitation of the bandwidth.<br>
 * <br>
 * This version shall not be in the same pipeline than other
 * TrafficShapingHandler.<br>
 * <br>
 * <p>
 * The general use should be as follow:<br>
 * <ul>
 * <li>Create your unique GlobalChannelTrafficShapingHandler like:<br>
 * <br>
 * <tt>GlobalChannelTrafficShapingHandler myHandler = new GlobalChannelTrafficShapingHandler(executor);</tt><br>
 * <br>
 * The executor could be the underlying IO worker pool<br>
 * <tt>pipeline.addLast(myHandler);</tt><br>
 * <br>
 * <p>
 * <b>Note that this handler has a Pipeline Coverage of "all" which means only
 * one such handler must be created and shared among all channels as the counter
 * must be shared among all channels.</b><br>
 * <br>
 * <p>
 ...
 * the doAccounting method (0 means no accounting at all).<br>
 * Note that as this is a fusion of both Global and Channel Traffic Shaping,
 * limits are in 2 sets, respectively Global and Channel.<br>
 * <br>
 * <p>
 ...
 * something close to 5 or 10 minutes.<br>
 * <br>
 * <p>
 ...
 * <li>You shall also consider to have object size in read or write operations
 ...
 * 1 MB/s should be smoothly handle by this TrafficShaping handler.<br>
 * <br>
 * </li>
 * <li>Some configuration methods will be taken as best effort, meaning that all
 ...
 * <br>
 * <p>
 * Be sure to call {@link #release()} once this handler is not needed anymore to
 * release all internal resources. This will not shutdown the {@link Timer} as
 * it may be shared, so you need to do this by your own.
 */
@Sharable
public class GlobalChannelTrafficShapingHandler extends AbstractTrafficShapingHandler {
    ...
    /**
     * Maximum cumulative writing bytes for one channel among all (as long as
     * channels stay the same)
     */
    private final AtomicLong cumulativeWrittenBytes = new AtomicLong();
    /**
     * Maximum cumulative read bytes for one channel among all (as long as
     * channels stay the same)
     */
    private final AtomicLong cumulativeReadBytes = new AtomicLong();
    /**
     * Max size in the list before proposing to stop writing new objects from
     * next handlers for all channel (global)
     */
     ...
     *
     * @param timer             the {@link Timer} to use for the {@link TrafficCounter}.
     ...
     * @param timer               the {@link Timer} to use for the {@link TrafficCounter}.
     * @param writeLimit          write Global Limit 0 or a limit in bytes/s
     * @param readLimit           read Global Limit 0 or a limit in bytes/s
     * @param writeChannelLimit   0 or a limit in bytes/s
     * @param readChannelLimit    0 or a limit in bytes/s
     * @param checkInterval       The delay between two computations of performances for
     *                            channels or 0 if no stats are to be computed.
     ...
     *                           default deviation being 0.1, so +/-10% of the desired
     *                           bandwidth. Maximum being 0.4.
     * @param slowDownFactor     the factor set as +x% to the too fast client (minimal value
     *                           being 0, meaning no slow down factor), default being 40%
     *                           (0.4).
     * @param accelerationFactor the factor set as -x% to the too slow client (maximal value
     *                           being 0, meaning no acceleration factor), default being -10%
     *                           (-0.1).
     */
    ...
    private void computeDeviationCumulativeBytes() {
        // compute the maximum cumulativeXxxxBytes among still connected
        // Channels
     ...
     * scheduled traffics will not be changed, but only applied to new
     * traffics.<br>
     * So the expected usage of this method is to be used not too often,
     * accordingly to the traffic shaping configuration.
     *
     * @param maxGlobalWriteSize the maximum Global Write Size allowed in the buffer globally
     *                           for all channels before write suspended is set.
     */
    ...
    private PerChannel getOrSetPerChannel(ChannelHandlerContext ctx) {
        ...
        if (perChannel == null) {
            ...
            perChannel.channelTrafficCounter = new TrafficCounter(this, null, "ChannelTC" + ctx.getChannel().hashCode(), checkInterval);
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent evt) throws Exception {
        ...
        try {
            ...
            // The message is then forcedly passed to the next handler (not to
            // super)
    ...
    protected void submitWrite(final ChannelHandlerContext ctx, final MessageEvent evt, final long size, final long writedelay, final long now) throws Exception {
        ...
        PerChannel perChannel = channelQueues.get(key);
        if (perChannel == null) {
            // in case write occurs before handlerAdded is raized for this
            // handler
