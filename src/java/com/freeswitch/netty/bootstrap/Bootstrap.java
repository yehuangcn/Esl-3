...
import static com.freeswitch.netty.channel.Channels.pipeline;
import static com.freeswitch.netty.channel.Channels.pipelineFactory;

/**
 * A helper class which initializes a {@link Channel}. This class provides the
...
public class Bootstrap implements ExternalResourceReleasable {
     ...
     * {@link #setFactory(ChannelFactory)} must be called at once before any I/O
    ...
    /**
     * Returns the {@link ChannelFactory} that will be used to perform an I/O
     * operation.
     *
     * @throws IllegalStateException if the factory is not set for this bootstrap yet. The factory
     *                               can be set in the constructor or
     ...
     * operation. This method can be called only once and can't be called at all
     * if the factory was specified in the constructor.
     *
     ...
     * {@link Channel}.
     * <p>
     * Please note that this method is a convenience method that works only when
     * <b>1)</b> you create only one channel from this bootstrap (e.g. one-time
     * client-side or connectionless channel) or <b>2)</b> all handlers in the
     * pipeline is stateless. You have to use
     ...
     * @throws IllegalStateException if {@link #setPipelineFactory(ChannelPipelineFactory)} was
     ...
     * internal {@link ChannelPipelineFactory} implementation which returns a
     * shallow copy of the specified pipeline.
     * <p>
     * Please note that this method is a convenience method that works only when
     * <b>1)</b> you create only one channel from this bootstrap (e.g. one-time
     * client-side or connectionless channel) or <b>2)</b> all handlers in the
     ...
     * {@link ChannelPipeline} for each new {@link Channel}. Calling this method
    ...
    /**
     * Returns the options which configures a new {@link Channel} and its child
     * {@link Channel}s. The names of the child {@link Channel} options are
     * prepended with {@code "child."} (e.g. {@code "child.keepAlive"}).
     */
    ...
    /**
     * Returns the value of the option with the specified key. To retrieve the
     * option value of a child {@link Channel}, prepend {@code "child."} to the
     * option name (e.g. {@code "child.keepAlive"}).
     *
     ...
     * prepend {@code "child."} to the option name (e.g.
