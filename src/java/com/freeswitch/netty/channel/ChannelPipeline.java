 ...
 * "http://java.sun.com/blueprints/corej2eepatterns/Patterns/InterceptingFilter.html">Intercepting
 ...
 * channel. Once attached, the coupling between the channel and the pipeline is
 * permanent; the channel cannot attach another pipeline to it nor detach the
 * current pipeline from it.
 * <p>
 ...
 * {@link ChannelHandler}s in a {@link ChannelPipeline} typically. A
 * {@link ChannelEvent} can be handled by either a
 * {@link ChannelUpstreamHandler} or a {@link ChannelDownstreamHandler} and be
 * forwarded to the closest handler by calling
 * {@link ChannelHandlerContext#sendUpstream(ChannelEvent)} or
 * {@link ChannelHandlerContext#sendDownstream(ChannelEvent)}. The meaning of
 * the event is interpreted somewhat differently depending on whether it is
 ...
 * usually handles the handler data generated by the I/O thread on the bottom of
 * the diagram. The handler data is often read from a remote peer via the actual
 * input operation such as {@link InputStream#read(byte[])}. If an upstream
 ...
 * usually generates or transforms the server traffic such as write requests.
 ...
 * <p>
 * For server, let us assume that we created the following pipeline:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * In the server above, the class whose name starts with {@code Upstream} means
 * it is an upstream handler. The class whose name starts with
 * {@code Downstream} means it is a downstream handler.
 * <p>
 * In the given server configuration, the handler evaluation order is 1, 2, 3,
 * 4, 5 when an event goes upstream. When an event goes downstream, the order is
 * 5, 4, 3, 2, 1. On top of this principle, {@link ChannelPipeline} skips the
 ...
 * therefore the actual evaluation order of a downstream event will be: 4 and
 * 3.</li>
 ...
 * evaluation order of an upstream and a downstream event could be 125 and 543
 * respectively.</li>
 * </ul>
 *
 ...
 * and close). For server, a typical server will have the following handlers in
 ...
 * </ol>
 *
 * and it could be represented as shown in the following server:
 *
 * <pre>
 ...
 * {@link ChannelPipeline} is thread safe. For server, you can insert a
 * {@link SslHandler} when sensitive information is about to be exchanged, and
...
public interface ChannelPipeline {
	 ...
	 *             if there's an entry with the same name already in the
	 *             pipeline
	 ...
	 *
	 * @throws NoSuchElementException
	 *             if there's no such handler with the specified name in this
	 *             pipeline
	 * @throws NullPointerException
	 ...
	 *
	 * @throws NoSuchElementException
	 *             if there's no such handler of the specified type in this
	 *             pipeline
	 * @throws NullPointerException
	 ...
	 * @throws IllegalArgumentException
	 *             if a handler with the specified new name already exists in
	 ...
	 *
	 * @throws NoSuchElementException
	 *             if the handler with the specified old name does not exist in
	 *             this pipeline
	 * @throws IllegalArgumentException
	 *             if a handler with the specified new name already exists in
	 *             this pipeline, except for the handler to be replaced
	 ...
	 *
	 * @throws NoSuchElementException
	 *             if the handler of the specified old handler type does not
	 ...
	 * pipeline.
	 *
	 * @return the handler with the specified name. {@code null} if there's no
	 ...
	 * pipeline.
	 *
	 * @return the handler of the specified handler type. {@code null} if
	 *         there's no such handler in this pipeline.
	 */
	 ...
	 * this pipeline.
	 *
	 * @return the context object of the specified handler. {@code null} if
	 *         there's no such handler in this pipeline.
	 */
	...
	/**
	 * Returns {@code true} if and only if this pipeline is attached to a
	 * {@link Channel}.
	 */
	boolean isAttached();
