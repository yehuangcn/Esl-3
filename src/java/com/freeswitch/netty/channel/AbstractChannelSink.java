...
public abstract class AbstractChannelSink implements ChannelSink {
	 ...
	 *
	 * @param event
	 *            the {@link ChannelEvent} which caused a {@link ChannelHandler}
	 *            to raise an exception
	 * @param cause
	 *            the exception raised by a {@link ChannelHandler}
	 */
	...
	/**
	 * Returns {@code true} if and only if the specified {@code actualCause},
	 * which was raised while handling the specified {@code event}, must trigger
	 * an {@code exceptionCaught()} event in an I/O thread.
	 *
	 * @param event
	 *            the event which raised exception
	 * @param actualCause
