 ...
 * variation of {@link ThreadLocal} whose key is a {@link Channel} rather than a
 * {@link Thread#currentThread()}. One difference is that you always have to
 * specify the {@link Channel} to access the variable.
 * <p>
 * Alternatively, you might want to use the
 * {@link ChannelHandlerContext#setAttachment(Object)
 * ChannelHandlerContext.attachment} property, which performs better.
 * 
...
public class ChannelLocal<T> implements Iterable<Entry<Channel, T>> {
	...
	/**
	 * Creates a {@link Channel} local variable by calling
	 * {@link #ChannelLocal(boolean)} with {@code false} as parameter
	 */
	public ChannelLocal() {
		this(false);
	}
	 ...
	 *
	 * @param removeOnClose
	 *            if {@code true} the {@link ChannelLocal} will remove a
	 *            {@link Channel} from it own once the {@link Channel} was
	 *            closed.
	 */
	 ...
	 *
	 * @return {@code null} if the specified value was set. An existing value if
	 *         failed to set.
	 */
	 ...
	 *
	 * @return the removed value. {@linkplain #initialValue(Channel) an initial
	 *         value} (by default {@code null}) if no value was set.
	 */
	...
	/**
	 * Returns a <strong>read-only</strong> {@link Iterator} that holds all
	 * {@link Entry}'s of this ChannelLocal
	 */
