...
public abstract class AbstractNioBossPool<E extends Boss> implements BossPool<E>, ExternalResourceReleasable {
	...
	/**
	 * The boss pool raises an exception unless all boss threads start and run
	 * within this timeout (in seconds.)
	 */
	...
	private void waitForBossThreads() {
		...
		for (Boss boss : bosses) {
			...
			try {
				...
				// Stop waiting for the boss threads and let someone else take
		...
		if (warn) {
			logger.warn("Failed to get all boss threads ready within " + INITIALIZATION_TIMEOUT + " second(s). " + "Make sure to specify the executor which has more threads than the requested bossCount. " + "If unsure, use Executors.newCachedThreadPool().");
		}
	}
