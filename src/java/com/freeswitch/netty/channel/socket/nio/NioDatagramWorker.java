...
import com.freeswitch.netty.channel.socket.nio.AbstractNioChannel.WriteRequestQueue;
...
/**
 * A class responsible for registering channels with {@link Selector}. It also
 * implements the {@link Selector} loop.
 */
public class NioDatagramWorker extends AbstractNioWorker {
     ...
     * Sole constructor.
     *
     * @param executor the {@link Executor} used to execute {@link Runnable}s such as
    ...
    protected boolean read(final SelectionKey key) {
        ...
        try {
            // Receive from the channel in a non blocking mode. We have already
            // been notified that
        ...
        if (remoteAddress != null) {
            ...
            if (readBytes > 0) {
                ...
                // Notify the interested parties about the newly arrived
                // message.
                fireMessageReceived(channel, buffer, remoteAddress);
            }
        }

        if (failure) {
            key.cancel(); // Some JDK implementations run into an infinite loop
            // without this.
            ...
            for (; ; ) {
                ...
                try {
                    ...
                    if (localWrittenBytes > 0 || buf.finished()) {
                        ...
                        // Not written at all - perhaps the kernel buffer is
                        // full.
                        addOpWrite = true;
