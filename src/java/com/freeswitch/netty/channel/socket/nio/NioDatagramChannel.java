...
/**
 * Provides an NIO based {@link com.freeswitch.netty.channel.socket.DatagramChannel}.
 */
public class NioDatagramChannel extends AbstractNioChannel<DatagramChannel> implements com.freeswitch.netty.channel.socket.DatagramChannel {
	...
	NioDatagramChannel(final ChannelFactory factory, final ChannelPipeline pipeline, final ChannelSink sink, final NioDatagramWorker worker, InternetProtocolFamily family) {
	...
	private static DatagramChannel openNonBlockingChannel(InternetProtocolFamily family) {
		try {
			...
			if (DetectionUtil.javaVersion() < 7 || family == null) {
				...
				// This block only works on java7++, but we checked before if we
				// have it.
				//
				// Use the ProtocolFamilyConvert for conversion to prevent
	...
	/**
	 * Joins the specified multicast group at the specified interface using the
	 * specified source.
	 */
	...
	/**
	 * Leave the specified multicast group at the specified interface using the
	 * specified source.
	 */
