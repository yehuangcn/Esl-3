...
import com.freeswitch.netty.channel.socket.DatagramChannelFactory;
import com.freeswitch.netty.channel.socket.InternetProtocolFamily;
import com.freeswitch.netty.channel.socket.Worker;
import com.freeswitch.netty.channel.socket.oio.OioDatagramChannelFactory;
 ...
 * {@link DatagramChannel}. It utilizes the non-blocking I/O mode which was
 * introduced with NIO to serve many number of concurrent connections
 * efficiently.
 * <p>
 * <h3>How threads work</h3>
 * <p>
 * There is only one thread type in a {@link NioDatagramChannelFactory}; worker
 * threads.
 * <p>
 * <h4>Worker threads</h4>
 * <p>
 * One {@link NioDatagramChannelFactory} can have one or more worker threads. A
 * worker thread performs non-blocking read and write for one or more
 * {@link DatagramChannel}s in a non-blocking mode.
 * <p>
...
public class NioDatagramChannelFactory implements DatagramChannelFactory {
    ...
    /**
     * Create a new {@link NioDatagramChannelFactory} with a
     * {@link Executors#newCachedThreadPool()} and without preferred
     * {@link InternetProtocolFamily}. Please note that the
     * {@link InternetProtocolFamily} of the channel will be platform (and
     * possibly configuration) dependent and therefore unspecified. Use
     * {@link #NioDatagramChannelFactory(InternetProtocolFamily)} if unsure.
     * <p>
     ...
     * <p>
     * Please note that the {@link InternetProtocolFamily} of the channel will
     * be platform (and possibly configuration) dependent and therefore
     * unspecified. Use
     * {@link #NioDatagramChannelFactory(Executor, InternetProtocolFamily)} if
     * unsure.
     *
     ...
     * be platform (and possibly configuration) dependent and therefore
     * unspecified. Use
     * {@link #NioDatagramChannelFactory(Executor, int, InternetProtocolFamily)}
     * if unsure.
     *
     * @param workerExecutor the {@link Executor} which will execute the I/O worker threads
     ...
     * be platform (and possibly configuration) dependent and therefore
     * unspecified. Use
     * {@link #NioDatagramChannelFactory(WorkerPool, InternetProtocolFamily)} if
     * unsure.
     *
     * @param workerPool the {@link WorkerPool} which will be used to obtain the
     *                   {@link NioDatagramWorker} that execute the I/O worker threads
     */
     ...
     *
     * @param workerExecutor the {@link Executor} which will execute the I/O worker threads
     * @param family         the {@link InternetProtocolFamily} to use. This should be used
     *                       for UDP multicast. <strong>Be aware that this option is only
     *                       considered when running on java7+</strong>
     */
     ...
     *                       considered when running on java7+</strong>
     */
    public NioDatagramChannelFactory(final Executor workerExecutor, final int workerCount, InternetProtocolFamily family) {
     ...
     *
     * @param workerPool the {@link WorkerPool} which will be used to obtain the
     *                   {@link Worker} that execute the I/O worker threads
     * @param family     the {@link InternetProtocolFamily} to use. This should be used
     *                   for UDP multicast. <strong>Be aware that this option is only
     *                   considered when running on java7+</strong>
     */
