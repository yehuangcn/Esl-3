...
abstract class AbstractOioWorker<C extends AbstractOioChannel> implements Worker {
	...
	public void run() {
		...
		// Mark the worker event loop as done so we know that we need to run
		// tasks directly and not queue them
		// See #287
		done = true;
	...
	public void executeInIoThread(Runnable task) {
		...
		//
		// Also check if the event loop of the worker is complete. If so we need
		// to run the task now.
		// See #287
		if (Thread.currentThread() == thread || done) {
			...
			if (added) {
				// as we set the SO_TIMEOUT to 1 second this task will get
				// picked up in 1 second at latest
			}
		}
	}
	 ...
	 * {@link Channels#fireMessageReceived(Channel, Object)} once a message was
	 * processed without errors.
	 *
	 * @return continue returns {@code true} as long as this worker should
	 *         continue to try processing incoming messages
	 * @throws IOException
	 */
			...
			if (channel.setClosed()) {
				...
				if (connected) {
					...
					synchronized (channel.interestOpsLock) {
						...
						// This can happen if run() set channel.workerThread to
						// null
						// between workerThread != null and currentThread!=
						// workerThread
						Thread workerThread = channel.workerThread;
						if (workerThread != null && currentThread != workerThread) {
							workerThread.interrupt();
