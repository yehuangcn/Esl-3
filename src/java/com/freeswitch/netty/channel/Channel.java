 ...
 * and bind), and</li>
 * <li>the {@link ChannelPipeline} which handles all {@linkplain ChannelEvent
 * I/O events and requests} associated with the channel.</li>
 * </ul>
 * <p>
 ...
 * implementation where the {@link Channel} belongs to. For server, you could
 ...
 * operations. For server, with the old I/O datagram transport, multicast join
 ...
 * which is similar to that of {@link SelectionKey#interestOps() NIO
 * SelectionKey}. It is represented as a
 * <a href="http://en.wikipedia.org/wiki/Bit_field">bit field</a> which is
 * composed of the two flags.
 * <ul>
 ...
 * peer until the {@link #OP_WRITE} flag is cleared and the write request will
 * be pending in a queue. If unset, the write request will be flushed out as
 * soon as possible from the queue.</li>
 ...
 * pending writes that lead to an {@link OutOfMemoryError}. For server, the NIO
 * socket transport uses the {@code writeBufferLowWaterMark} and
...
public interface Channel extends Comparable<Channel> {
     ...
     *
     * @return the local address of this channel. {@code null} if this channel
     * is not bound.
     */
     ...
     * information.
     *
     * @return the remote address of this channel. {@code null} if this channel
     * is not connected. If this channel is not connected but it can
     * receive messages from arbitrary remote addresses (e.g.
     * {@link DatagramChannel}, use
     * {@link MessageEvent#getRemoteAddress()} to determine the
     * origination of the received message as this method will return
     * {@code null}.
     */
     ...
     * created by a connectionless transport (e.g. {@link DatagramChannel}) and
     * is not connected yet, you have to call
     * {@link #write(Object, SocketAddress)} instead. Otherwise, the write
     * request will fail with {@link NotYetConnectedException} and an
     ...
     * message instead of this channel's current remote address. If this channel
     * was created by a connectionless transport (e.g. {@link DatagramChannel})
     * and is not connected yet, you must specify non-null address. Otherwise,
     * the write request will fail with {@link NotYetConnectedException} and an
     * {@code 'exceptionCaught'} event will be triggered.
     *
     * @param message       the message to write
     * @param remoteAddress where to send the specified message. This method is identical
     *                      to {@link #write(Object)} if {@code null} is specified here.
     * @return the {@link ChannelFuture} which will be notified when the write
     * request succeeds or fails
     * @throws NullPointerException if the specified message is {@code null}
     */
     ...
     * @return the {@link ChannelFuture} which will be notified when the bind
     * request succeeds or fails
     ...
     * connected, it will be disconnected and unbound first. Once a channel is
     * closed, it can not be open again. Calling this method on a closed channel
     * has no effect. Please note that this method always returns the same
     * future instance.
     *
     * @return the {@link ChannelFuture} which will be notified when the close
     * request succeeds or fails
     */
    ChannelFuture close();
     ...
     * requested write operation immediately. Any write requests made when this
     * method returns {@code false} are queued until the I/O thread is ready to
     * process the queued write requests. This method is a shortcut to the
     * following code:
     * <p>
     * <pre>
     ...
     * </pre>
     *
     * @param readable {@code true} to resume the read operation and {@code false} to
